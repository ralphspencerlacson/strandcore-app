...
export default function Navbar({ loading = false, contactPage = false }: { loading?: boolean; contactPage?: boolean }) {
  ...
  return (
    <header className={`site-header${scrolled ? ' is-scrolled' : ''}${loading ? ' is-loading' : ''}`} inert={loading}>
      <nav className="site-nav" aria-label="Main navigation">
        <a className="brand-link" href={contactPage ? '/#top' : '#top'} onClick={() => setOpen(false)} aria-label="Strandcore home"><Brand /></a>
        ...
        <div id="navigation-links" className={`nav-links${open ? ' is-open' : ''}`}>
          {links.map(link => <a key={link} href={link === 'Contact' ? '/contact' : `${contactPage ? '/' : ''}#${link.toLowerCase()}`} aria-current={contactPage ? (link === 'Contact' ? 'page' : undefined) : active === link ? 'location' : undefined} onClick={() => setOpen(false)}>{link}</a>)}
